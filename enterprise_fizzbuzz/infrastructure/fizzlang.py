"""
Enterprise FizzBuzz Platform - FizzLang Domain-Specific Language & Formal Grammar
...
Also provides FizzGrammar: a BNF/EBNF grammar specification engine with
FIRST/FOLLOW set computation, LL(1) classification, left-recursion
detection, bounded ambiguity analysis, unreachable symbol detection,
a parser generator that compiles grammars to recursive-descent parsers,
and a Grammar Health Index dashboard.
...
# ======================================================================
#
# Everything below this line was originally in formal_grammar.py.
# It provides BNF/EBNF grammar specification, FIRST/FOLLOW set
# computation, LL(1) classification, left-recursion detection,
# ambiguity analysis, parser generation, and a Grammar Health Index
# dashboard. The curation audit merged it here because fizzlang.py's
# Turing-incompleteness thesis is sharper, and both modules address
# the same satirical premise: formal language theory applied to
# a problem that does not require it.
# ======================================================================

# ============================================================
# Sentinel constants (FizzGrammar)
# ============================================================

EPSILON = "\u03b5"
EOF_SENTINEL = "$"

# Re-export under the name tests expect
EOF = EOF_SENTINEL


# ============================================================
# Symbol types (FizzGrammar)
# ============================================================
...
# ============================================================
# FIRST / FOLLOW set computation (FizzGrammar)
# ============================================================
...
# ============================================================
# Left Recursion Detector (FizzGrammar)
# ============================================================
...
# ============================================================
# Ambiguity Analyzer (FizzGrammar)
# ============================================================
...
# ============================================================
# Unreachable Symbol Detector (FizzGrammar)
# ============================================================
...
# ============================================================
# Grammar AST Node (FizzGrammar)
# ============================================================


@dataclass
class GrammarASTNode:
    """A generic Abstract Syntax Tree node for generated parsers.
    ...
    Named ``GrammarASTNode`` to distinguish from FizzLang's ``ASTNode``.
    """

    node_type: str
    children: list["GrammarASTNode"] = field(default_factory=list)
...
# ============================================================
# Grammar Token (FizzGrammar)
# ============================================================


@dataclass
class GrammarToken:
    """A lexical token produced by the grammar tokenizer.

    Named ``GrammarToken`` to distinguish from FizzLang's ``Token``.
    """
...
class GeneratedParser:
    ...
    def __init__(self) -> None:
        self._tokens: list[GrammarToken] = []
        self._pos: int = 0
        self._errors: list[dict[str, Any]] = []

    def parse(self, text: str) -> GrammarASTNode:
        ...
        while i < len(text):
            ...
            for kind, pattern in self._token_patterns:
                if pattern.startswith("__re__"):
                    ...
                    if m:
                        val = m.group()
                        self._tokens.append(GrammarToken(kind, val, line, col))
                        col += len(val)
                        i += len(val)
                        matched = True
                        break
                    ...
                    if text[i:i + len(pattern)] == pattern:
                        ...
                        if pattern.isalpha() and end < len(text) and (
                            ...
                            continue
                        self._tokens.append(GrammarToken(kind, pattern, line, col))
                        col += len(pattern)
                        i += len(pattern)
                        matched = True
                        break
            if not matched:
                ...
                i += 1
        # Append EOF token
        self._tokens.append(GrammarToken(EOF_SENTINEL, "", line, col))

    def _peek(self) -> GrammarToken:
        if self._pos < len(self._tokens):
            return self._tokens[self._pos]
        return GrammarToken(EOF_SENTINEL, "", 0, 0)

    def _consume(self, expected_kind: Optional[str] = None) -> GrammarToken:
        tok = self._peek()
    ...
    def _report_error(self, tok: GrammarToken, expected: list[str]) -> None:
        error_info = {
            "line": tok.line,
            "column": tok.column,
            "found": tok.kind,
    ...
    def _recover(self) -> None:
        ...
        while self._peek().kind != EOF_SENTINEL:
            if self._peek().value in self._sync_tokens or self._peek().kind in self._sync_tokens:
                return
            self._pos += 1

    def _parse_non_terminal(self, nt_name: str) -> GrammarASTNode:
        ...
        for prod in prods:
            if self._is_nullable_production(prod):
                return GrammarASTNode(node_type=nt_name, line=lookahead.line, column=lookahead.column)
    ...
    def _can_start(self, prod: Production, lookahead: GrammarToken) -> bool:
    ...
    def _parse_production(self, prod: Production) -> GrammarASTNode:
        ...
        for sym in prod.rhs:
            ...
            if sym.is_terminal:
                ...
                if tok.kind == sym.name or tok.value == sym.name:
                    self._pos += 1
                    node.children.append(GrammarASTNode(
                        node_type=sym.name,
                        token=tok.value,
                        line=tok.line,
                        column=tok.column,
...
# ============================================================
# Grammar Analysis Report (FizzGrammar)
# ============================================================
...
class GrammarDashboard:
    ...
    def render(grammar: Grammar, analyzer: Optional[GrammarAnalyzer] = None, width: int = 60) -> str:
        ...
        border = "+" + "-" * (width - 2) + "+"
        lines.append(border)
        lines.append(_grammar_center("FizzGrammar: Formal Grammar & Parser Generator", width))
        lines.append(_grammar_center("Chomsky hierarchy compliance since 1956", width))
        lines.append(border)

        # Grammar inventory
        lines.append(_grammar_center("GRAMMAR INVENTORY", width))
        lines.append(border)
        stats = report["statistics"]
        lines.append(_grammar_row("Grammar", stats["name"], inner, width))
        lines.append(_grammar_row("Terminals", str(stats["terminals"]), inner, width))
        lines.append(_grammar_row("Non-terminals", str(stats["non_terminals"]), inner, width))
        lines.append(_grammar_row("Productions", str(stats["productions"]), inner, width))
        lines.append(_grammar_row("Grammar class", report["grammar_class"], inner, width))
        lines.append(border)

        # FIRST sets (abbreviated)
        lines.append(_grammar_center("FIRST SETS", width))
        lines.append(border)
        for nt, first in sorted(report["first_sets"].items()):
            ...
            lines.append(_grammar_row(nt, "{ " + first_str + " }", inner, width))
        lines.append(border)

        # FOLLOW sets (abbreviated)
        lines.append(_grammar_center("FOLLOW SETS", width))
        lines.append(border)
        for nt, follow in sorted(report["follow_sets"].items()):
            ...
            lines.append(_grammar_row(nt, "{ " + follow_str + " }", inner, width))
        lines.append(border)

        # Diagnostics
        lines.append(_grammar_center("DIAGNOSTICS", width))
        lines.append(border)
        ...
        lines.append(_grammar_row("LL(1)", ll1_status, inner, width))
        lines.append(_grammar_row("Left recursion", lr_status, inner, width))
        lines.append(_grammar_row("Ambiguity", amb_status, inner, width))
        lines.append(_grammar_row("Unreachable", unreach, inner, width))
        lines.append(border)

        # Health Index
        lines.append(_grammar_center("GRAMMAR HEALTH INDEX", width))
        lines.append(border)
        health = report["health_index"]
        bar_width = inner - 10
        filled = int(health / 100 * bar_width)
        bar = "#" * filled + "-" * (bar_width - filled)
        lines.append(_grammar_row("Health", f"{health}% [{bar}]", inner, width))
        lines.append(border)

        return "\n".join(lines)


def _grammar_center(text: str, width: int) -> str:
    """Center text in a grammar dashboard row."""
    padded = text.center(width - 4)
    return f"| {padded} |"


def _grammar_row(label: str, value: str, inner: int, width: int) -> str:
    """Format a label-value row for the grammar dashboard."""
