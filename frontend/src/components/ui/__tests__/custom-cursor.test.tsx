...
describe("CustomCursor", () => {
  ...
  it("returns null when not visible", () => {
    mockUseCursor.mockReturnValue({ x: 0, y: 0, cursorState: "default", visible: false });
    const { container } = render(<CustomCursor />);
    expect(container.firstChild).toBeNull();
  });

  it("renders with fixed positioning and pointer-events-none", () => {
    mockUseCursor.mockReturnValue({ x: 50, y: 50, cursorState: "default", visible: true });
    const { container } = render(<CustomCursor />);
  ...
  it("renders pointer cursor as 32px ring", () => {
    mockUseCursor.mockReturnValue({ x: 50, y: 50, cursorState: "pointer", visible: true });
    const { container } = render(<CustomCursor />);
    const inner = container.querySelector(".custom-cursor-container > div")!;
  ...
  it("renders text cursor as 2px vertical bar", () => {
    mockUseCursor.mockReturnValue({ x: 50, y: 50, cursorState: "text", visible: true });
    const { container } = render(<CustomCursor />);
    const inner = container.querySelector(".custom-cursor-container > div")!;
