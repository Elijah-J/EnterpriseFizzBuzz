...
export default function AlertsPage() {
  ...
  const toggleSilenced = useCallback((alertId: string) => {
    setAlerts((prev) =>
      prev.map((a) =>
        a.id === alertId ? { ...a, silenced: !a.silenced } : a,
      ),
    );
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Summary bar */}
      <div className="flex flex-wrap items-center gap-4 rounded-lg border border-panel-700 bg-panel-800 px-4 py-3">
        ...
        <div className="flex items-center gap-3 text-xs text-panel-400">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="rounded border border-panel-700 bg-panel-800 px-2.5 py-1 text-xs text-panel-200 outline-none focus:border-panel-500"
        >
        ...
        <div className="flex items-center gap-3 ml-auto">
          <label className="flex items-center gap-1.5 text-xs text-panel-400 cursor-pointer">
            <input
              type="checkbox"
              checked={showAcknowledged}
              onChange={(e) => setShowAcknowledged(e.target.checked)}
              className="rounded border-panel-600 bg-panel-900 text-fizz-400 focus:ring-0 focus:ring-offset-0 h-3 w-3"
            />
            Show Acknowledged
          </label>
          <label className="flex items-center gap-1.5 text-xs text-panel-400 cursor-pointer">
      ...
      <div className="space-y-3">
        {filteredAlerts.length === 0 && (
          <div className="flex h-32 items-center justify-center rounded-lg border border-panel-700 bg-panel-800">
            <span className="text-xs text-panel-500">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-1.5">
                      ...
                      <span className="rounded bg-panel-900 px-1.5 py-0.5 text-[10px] text-panel-400 border border-panel-700">
                        {alert.subsystem}
                      </span>
                      {alert.acknowledged && (
                        <span className="rounded bg-panel-900 px-1.5 py-0.5 text-[10px] text-panel-500 border border-panel-700">
                          Acknowledged
                        </span>
                      )}
                      {alert.silenced && (
                    ...
                    <h3 className="text-sm font-medium text-panel-100">
                      {alert.title}
                    </h3>
                    <p className="text-xs text-panel-400 mt-1 leading-relaxed">
                      {alert.description}
                    </p>

                    {/* Timestamp */}
                    <p className="text-[10px] text-panel-500 mt-2">
                    ...
                    <button
                      ...
                      className={`rounded px-2.5 py-1 text-[10px] font-medium border transition-colors ${
                        alert.acknowledged
                          ...
                          : "bg-panel-900 text-panel-400 border-panel-700 hover:border-panel-500 hover:text-panel-200"
                      }`}
                    >
